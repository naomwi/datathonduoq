...
from make_publiconly_sample_scale_ratio_v23 import global_scale, write_and_summarize
from run_transaction_decomposition_v2 import DATASET_DIR, LOG_ROOT, NOTES_DIR


RUN_PREFIX = "publiconly_sample_cogs_inverse_v26"
...
def cogs_ratio_away_from_sample(current: pd.DataFrame, sample: pd.DataFrame, alpha: float) -> pd.DataFrame:
    ...
    for period, idx in cur_prof.groupby("period").groups.items():
        ...
        target_ratio = cur_ratio + alpha * (cur_ratio - sample_ratio)
...
def main() -> None:
    ...
    specs = [
        (
            "submission_sample_v26_a0725_ratio_away_sample0025.csv",
            cogs_ratio_away_from_sample(current, sample, 0.025),
            "keep Revenue/day-shape, move COGS period ratios 2.5% away from sample",
            1,
        ),
        (
            "submission_sample_v26_a0725_ratio_away_sample0050.csv",
            cogs_ratio_away_from_sample(current, sample, 0.050),
            "keep Revenue/day-shape, move COGS period ratios 5% away from sample",
            2,
        ),
        (
            "submission_sample_v26_a0725_ratio_away_sample0075.csv",
            cogs_ratio_away_from_sample(current, sample, 0.075),
            "keep Revenue/day-shape, move COGS period ratios 7.5% away from sample",
            3,
        ),
        (
            "submission_sample_v26_a0725_ratio_away_sample0100.csv",
            cogs_ratio_away_from_sample(current, sample, 0.100),
            "keep Revenue/day-shape, move COGS period ratios 10% away from sample",
            4,
        ),
        (
            "submission_sample_v26_a0725_cogsscale_up0025.csv",
            global_scale(current, revenue_multiplier=1.0, cogs_multiplier=1.0025),
            "global COGS +0.25%, keep Revenue",
            5,
        ),
        (
            "submission_sample_v26_a0725_cogsscale_up0050.csv",
            global_scale(current, revenue_multiplier=1.0, cogs_multiplier=1.0050),
            "global COGS +0.5%, keep Revenue",
            6,
        ),
        (
            "submission_sample_v26_a0725_cogsscale_up0100.csv",
            global_scale(current, revenue_multiplier=1.0, cogs_multiplier=1.0100),
            "global COGS +1.0%, keep Revenue",
            7,
        ),
    ]
    ...
    report = f"""# Public-Only Sample COGS Inverse V26
...
"""
    ...
    (NOTES_DIR / "publiconly_sample_cogs_inverse_v26_2026-04-22.md").write_text(report, encoding="utf-8")
    print(run_dir)
