...
RUN_PREFIX = "quarantine_blackbox60_v9_jump_continue"
CURRENT_BEST_FILE = "submission_qbb60v8_nonh2shape_2023h1level105_away0300.csv"
CURRENT_BEST_SCORE = 668570.18037
...
def specs() -> list[Spec]:
    ...
    return [
        Spec(
            "qbb60v9_nonh2shape_2023h1level107_away0300",
            base_rev,
            0.300,
            1.070,
            1.030,
            1.000,
            "Continue the proven 2023H1 level axis from +5% to +7% while keeping the current working away regime.",
        ),
        Spec(
            "qbb60v9_nonh2shape_2023h1level110_away0300",
            base_rev,
            0.300,
            1.100,
            1.030,
            1.000,
            "High-variance continuation of the 2023H1 level axis to +10%.",
        ),
        Spec(
            "qbb60v9_nonh2shape_2023h1level107_away0325",
            base_rev,
            0.325,
            1.070,
            1.030,
            1.000,
            "Combine the two positive directions: more 2023H1 level and slightly stronger away.",
        ),
        Spec(
            "qbb60v9_nonh2shape_2023h1level105_away0350",
            base_rev,
            0.350,
            1.050,
            1.030,
            1.000,
            "Keep the proven +5% 2023H1 level and test whether away still has substantial headroom.",
        ),
        Spec(
            "qbb60v9_nonh2shape_2023h1level107_away0300_2024h1p1200",
            {"2023H1": 0.900, "2023H2": 0.100, "2024H1": 1.200},
            0.300,
            1.070,
            1.030,
            1.000,
            "Full revenue-side jump attempt: if 2023H1 level and 2024H1 shape both still have room simultaneously.",
        ),
    ]
...
def write_report(run_dir: Path, manifest: pd.DataFrame) -> None:
    report = f"""# Quarantine Blackbox 60x V9 Jump Continue
...
"""
    ...
    (NOTES_DIR / "quarantine_blackbox60_v9_jump_continue_2026-04-24.md").write_text(report, encoding="utf-8")
