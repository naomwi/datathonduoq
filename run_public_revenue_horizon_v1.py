"""
Evaluate Revenue Horizon-binned models.
Model A (Early) is used for forecast_step <= cut.
Model B (Late) is used for forecast_step > cut.
Both use the recencyexp20 architecture, but Model B uses stronger regularization (depth=4) to handle recursive noise.
"""
...
from run_leaderboard_sprint import export_final_submission

RUN_PREFIX = "public_revenue_horizon_v1"
DATASET_DIR = Path("dataset")

CANDIDATES = [
    {
        "candidate_id": "revenue_horizon_v1_cut15",
        "horizon_cut": 15,
    ...
    {
        "candidate_id": "revenue_horizon_v1_cut30",
        "horizon_cut": 30,
        "revenue_experiment": "curated_promo_cogs",
        "cogs_experiment": "curated_promo_cogs",
        "promo_future_policy": "seasonal_month_day_recent_2y",
    ...
    {
        "candidate_id": "revenue_horizon_v1_cut45",
        "horizon_cut": 45,
        "revenue_experiment": "curated_promo_cogs",
        "cogs_experiment": "curated_promo_cogs",
        "promo_future_policy": "seasonal_month_day_recent_2y",
...
def recursive_horizon_forecast(
    ...
    # Model A: Early Horizon (Defaults to CatBoost standard: depth=6, l2_leaf_reg=3.0)
    # Model B: Late Horizon (Stronger regularized to handle recursive synthetic lags: depth=4, l2_leaf_reg=10.0)
    early_params = {}
    late_params = {"depth": 4, "l2_leaf_reg": 10.0}

    revenue_model_early = make_regressor("catboost", model_params_override=early_params)
    revenue_model_late = make_regressor("catboost", model_params_override=late_params)
    cogs_model = make_regressor("catboost")

    print(f"    -> Fitting Model A (Early, depth=6)")
    fit_regressor(
        revenue_model_early,
    ...
    print(f"    -> Fitting Model B (Late, depth=4, l2_leaf_reg=10.0)")
    fit_regressor(
        revenue_model_late,
        train_df[revenue_features],
        _transform_target_series(train_df["Revenue"], "none"),
    ...
    for horizon_index, current_date in enumerate(forecast_dates):
        ...
        active_revenue_model = revenue_model_early if forecast_step <= horizon_cut else revenue_model_late
...
def build_summary(fold_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Anchor definition: recencyexp20 had ~603855 combined OOF.
...
def write_report(run_dir: Path, summary_df: pd.DataFrame, fold_df: pd.DataFrame, top_ids: list[str]) -> None:
    ...
    with report_path.open("w", encoding="utf-8") as f:
        f.write("# Revenue Horizon V1 Sprint\n\n")
        f.write("## Hypothesis\n")
        f.write("- Model A (standard CatBoost) handles early recursive rollout.\n")
        f.write("- Model B (high regularization depth=4) handles late recursive rollout to mitigate drift.\n")
        f.write("- Routing strictly by `forecast_step`.\n\n")
...
def main() -> None:
    ...
    logger.info("Starting public revenue horizon v1 sprint in %s", run_dir)
    ...
    # We submit if beats anchor OR just the best one if none beat
