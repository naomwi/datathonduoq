"""
True Direct Multi-Horizon CatBoost (v1)
Unrolls the dataset via f(X_t, Covariates_{t+h}, h) = Y_{t+h} to completely bypass recursive error loops.
Evaluates explicit horizon cuts (15, 30, 45) for Early and Late Direct models using strict_core + promo_detail.
"""
...
RUN_PREFIX = "public_revenue_direct_horizon_v1"
DATASET_DIR = Path("dataset")

# The precise gating parameters required by the user spec.
CANDIDATES = [
    {"candidate_id": "direct_horizon_v1_cut15", "cut": 15},
    {"candidate_id": "direct_horizon_v1_cut30", "cut": 30},
    {"candidate_id": "direct_horizon_v1_cut45", "cut": 45}
]

def evaluate_anchor(feature_store: pd.DataFrame, base: pd.DataFrame, feature_sets: dict) -> pd.DataFrame:
    """ Evaluates the recencyexp20 anchor on the fly to grab exact OOF COGS and Revenue baselines for gating. """
    results = []
...
def build_unrolled_dataset(feature_store: pd.DataFrame, base: pd.DataFrame, features: list[str], max_h: int):
    # Future cols are basically Promos and Calendar flags
    future_indicators = PROMO_RAW_COLUMNS + ["sin_", "cos_", "is_", "year", "month", "day"]
    future_cols = [f for f in features if any(x in f for x in future_indicators) and "lag_" not in f and "rollmean" not in f and "ewm_" not in f]
    ...
    # Restore string dtype for CatBoost object features
...
def direct_multi_horizon_evaluate(unrolled_train, origin_cols, future_cols, features, cut, train_end_date, forecast_start, forecast_end, feature_store, base, anchor_cogs):
    train_end_date = pd.Timestamp(train_end_date)
    forecast_start = pd.Timestamp(forecast_start)
    forecast_end = pd.Timestamp(forecast_end)
    
    years_from_end = np.maximum((train_end_date - unrolled_train["target_date"]).dt.days / 365.25, 0.0)
    # same recency decay as recencyexp20
    ...
    fit_regressor(
        early_model,
        unrolled_train.loc[early_mask, train_features],
        _transform_target_series(unrolled_train.loc[early_mask, "y_target"], "none"),
        "catboost",
        sample_weight=unrolled_train.loc[early_mask, "sample_weight"]
    )
    ...
    if late_mask.any():
        ...
        fit_regressor(
            late_model,
            unrolled_train.loc[late_mask, train_features],
            _transform_target_series(unrolled_train.loc[late_mask, "y_target"], "none"),
            "catboost",
            sample_weight=unrolled_train.loc[late_mask, "sample_weight"]
        )
        
    print(f"      Inference Direct Sequence...")
    ...
    infer_df = pd.DataFrame(index=forecast_dates)
    infer_df["forecast_step"] = np.arange(1, len(forecast_dates) + 1)
    ...
    for c in future_cols:
        if c in base_idx.columns:
            infer_df[c] = base_idx.reindex(forecast_dates)[c].values
        elif c in fs_idx.columns:
            infer_df[c] = fs_idx.reindex(forecast_dates)[c].values
            
    # Fix category types
    for c in train_features:
        if unrolled_train[c].dtype == object or isinstance(unrolled_train[c].dtype, pd.CategoricalDtype):
            infer_df[c] = infer_df[c].astype(str)
            
    preds = np.zeros(len(infer_df))
    inf_early_mask = infer_df["forecast_step"] <= cut
    inf_late_mask = infer_df["forecast_step"] > cut
    
    if inf_early_mask.any():
        p_res = early_model.predict(infer_df.loc[inf_early_mask, train_features])
        preds[inf_early_mask] = [_inverse_transform_scalar(float(x), "none") for x in p_res]
    if inf_late_mask.any():
        p_res = late_model.predict(infer_df.loc[inf_late_mask, train_features])
        preds[inf_late_mask] = [_inverse_transform_scalar(float(x), "none") for x in p_res]
        
    preds = np.maximum(preds, 0.0)
    
    cogs_mapping = anchor_cogs.set_index("Date")["COGS_pred"].to_dict()
    cogs_preds = [cogs_mapping.get(td, 0.0) for td in forecast_dates]
...
def main():
    ...
    logger.info("Initializing Direct Multi-Horizon CatBoost offline evaluation...")
    ...
    if not features:
        # Reconstruct if missing (sometimes it's generated dynamically in core_plus script)
    ...
    logger.info("Step 1: Generating Anchor OOF predictions (recencyexp20) to extract lock-in COGS...")
    anchor_oof = evaluate_anchor(feature_store, base, feature_sets)
    anchor_oof["Date"] = pd.to_datetime(anchor_oof["Date"])
    
    # Unroll the dataset for Direct Multi-Horizon testing
    logger.info("Step 2: Unrolling Dataset to strict Origin -> Y(t+h) format...")
    # Max horizon is max length of any backtest fold
    max_h = max([(pd.Timestamp(f[1]) - pd.Timestamp(f[0])).days + 1 for f in BACKTEST_FOLDS])
    unrolled_full, origin_cols, future_cols = build_unrolled_dataset(feature_store, base, features, max_h=max_h)
    
    logger.info(f"Unrolled Dataset crafted: {len(unrolled_full):,} rows. Future Cols: {len(future_cols)}, Origin Lags: {len(origin_cols)}")
    ...
    for candidate in CANDIDATES:
        ...
        cut = candidate["cut"]
        logger.info(f"\n--- Evaluating Candidate: {c_id} (Cut={cut}) ---")
        ...
        for fold_id, (start_date, end_date) in enumerate(BACKTEST_FOLDS, start=1):
            ...
            preds = direct_multi_horizon_evaluate(
    ...
    for c_id in [c["candidate_id"] for c in CANDIDATES]:
        ...
        # Weighted combined recent
        # Approx weighting: standard mean for simplicity or we can weight fold 3 more?
        # Typically "recent-weighted combined" averages fold 2+3. We just use the concat of fold 2+3.
        c_recent_weighted, _ = compute_metrics(c_recent)
        anc_recent_weighted, _ = compute_metrics(anc_recent)
        
        # Recent late-horizon Revenue (h > cut)
        ...
        gate_reports.append({
            "candidate_id": c_id,
            "gate_pass": gate_pass,
            "c_combined_oof": c_combined_oof,
            "anchor_combined_oof": anc_combined_oof,
            "c_recent_weighted": c_recent_weighted,
            "anc_recent_weighted": anc_recent_weighted,
            "c_late_rev": c_late_rev,
            "anc_late_rev": anc_late_rev
        })
    ...
    if passed_ids:
        logger.info(f"Candidates passing export gate: {passed_ids}")
        # Build Export (Public Submission)
        # Load COGS from submission
        try:
            ...
            logger.warning("Could not find public recencyexp20 submission. COGS will be 0.")
            pub_cogs = None
            
        unrolled_full_inf = unrolled_full[unrolled_full["target_date"] <= pd.Timestamp(TRAIN_END)].copy()
        
        for c_id in passed_ids:
            cut_val = next(c["cut"] for c in CANDIDATES if c["candidate_id"] == c_id)
            adj_base = apply_future_promo_policy(base, TRAIN_END, "seasonal_month_day_recent_2y")
            
            # anchor_cogs for direct_multi_horizon_evaluate
            fake_anchor = pub_cogs.rename(columns={"COGS": "COGS_pred"}) if pub_cogs is not None else pd.DataFrame(columns=["Date", "COGS_pred"])
            
            pub_preds = direct_multi_horizon_evaluate(
                unrolled_full_inf, origin_cols, future_cols, features, cut_val,
                TRAIN_END, "2023-01-01", "2024-07-01", feature_store, adj_base, fake_anchor
            )
        ...
        logger.info("No candidates passed the strict gating requirements.")
